...
## RLS Summary (migration 20260910092000)
...
## Known Limitations
1. جداول المجتمع **غير مطبقة بعد** على قاعدة الإنتاج — المالك يطبق 3 migrations عبر SQL
   Editor (لا مسار DDL من البيئة). حتى ذلك الحين تعرض صفحات المجتمع حالات «لم تُفعّل»
   المصممة، والموقع الأساسي سليم 100%.
2. اختبارات RLS الحية وسيناريوهات E2E الكاملة بانتظار (1) + env.
3. إنشاء دورة اختبار مستخدمين حقيقيين يتطلب حسابات Supabase — سكربت SQL يُنشئها ويحذفها.
4. التعليقات على المنشور المخفي تُرى لصاحب المنشور فقط (سياسة select) — تصميم مقصود.
