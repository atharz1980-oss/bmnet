...
**D-99. بناء إنتاجي نظيف** — بناء Turbopack فوق كاش `.next` دافئ أنتج HTML مُزروعًا يشير إلى chunks غير مُصدَّرة → انهيار client-side بذاكرة باردة على /admin/login و/contact و/corporate-training (الخادم يرجع 500 للملف الثابت المفقود). الإصلاح: `rm -rf .next` في سكربت البناء (`package.json`) قبل `next build` — كل بناء إنتاجي يبدأ من صفر، فلا تُشير المخرجات المزروعة أبدًا إلى chunks غير موجودة.
...
**D-100. معمارية المجتمع (CP-H V1)** — هوية العضو = `auth.users` + `community_profiles` (لا لمس profiles الموظفين)؛ كل كتابة عضو عبر عميل الكوكيز فيطبّق RLS (لا عميل خدمة في مسار العضو)؛ الإشراف بوحدة `community` في `admin_module` عبر `has_permission`؛ الإشعارات بمشغلات security definer مع block_cleanup يقطع المتابعة المتبادلة؛ bucket مستقل `community-media` بمجلد لكل عضو (`community/{uid}/`)؛ الحجب المتبادل يُفحص داخل سياسات like/comment/follow؛ المالك لا يُحيي منشورًا مخفيًا إداريًا (update own يقيد status=published)؛ محمّلات عامة tolerate (D-86) فلا انهيار قبل تطبيق المخططات؛ بوابة ناعمة `requireCommunityUser` لإنشاء الملف (كسر الدائرة القاهرة: لا ملف ← لا بوابة ملف).
