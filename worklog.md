...
## Task 44 — اختبار قبول الإنتاج الحي (PAT)

نُفِّذ كاملًا على `baytalmosawer.net` لا على نسخة محلية. الحسابات المؤقتة أُنشئت إداريًا (بلا بريد) وحُذفت كلها.

- **الموقع العام 39/40**: 18 صفحة تُفتح بلا حساب وبلا تحويل، 29 رابطًا داخليًا سليمًا، 18 صورة عبر المحسِّن، robots وsitemap نظيفان، 404 صحيح، صفر أخطاء وحدة تحكم على 6 مسارات.
- **اللوحة ← الموقع الحي**: التعديل يمر بـServer Action الحقيقي بجلسة إدارة حقيقية (بوابة `requirePermission`)، ثم يُقرأ الموقع العام. **أُثبتت الدورة الكاملة (حفظ ← قاعدة ← ظهور حي ← استرجاع حرفي) لأربعة أنواع: الإعدادات العامة، بيانات التواصل، وسائل التواصل، ومقال المدونة.** تحرير الدورة لم يُثبت: هيكل الاختبار يعيد بناء حمولة `CourseInput` كاملة من صفوف القاعدة الخام، فارتدّ بخطأ قيد مرجعي على المواعيد — لا دليل على عطل في المنتج ولا دليل على سلامته، فيبقى ضمن ما يؤكده المالك بنفسه من الواجهة. **ملاحظة تشغيلية: `/courses` و`/about` تعكسان التعديل خلال ثانية، بينما الرئيسية قد تتأخر حتى 5 دقائق** — CDN هوستنجر يخدمها بـ`s-maxage=300`. قِيس: ظهرت خلال ثانية واحدة عند أول طلب بعد انتهاء الكاش.
- **رحلة المصور 38/45 ثم إكمال**: ملف كامل (اسم مستخدم، اسم معروض، نبذة، تخصصات، مدينة ودولة، صورة وغلاف) وتعديله، ورفض اسم مستخدم مكرر، ومشروعا بورتفوليو بثلاث وسائط، والملف العام يعرضها — كلها عبر مسار التطبيق لا رابط تخزين. الإخفاقات السبعة كانت في هيكل الاختبار (اسم عمود `user_id` لا `owner_id`، توقيع البلاغ، ومعرّفات إجراءات في حزم صفحات أخرى) لا في المنتج، وأُعيدت كلها بنجاح.
- **النشر والتفاعل**: منشور بصورة، إعجاب وإلغاؤه، تعليق ورد، حفظ وإلغاؤه، متابعة وإلغاؤها بتأكيد الصفوف، إشعارات، اكتشاف، بلاغ، وحجب يقطع المتابعة ويمنع المحجوب من المتابعة والتعليق، وفك حجب يعيد الأمر. وصلاحيات متقاطعة: لا يحذف عضو منشور غيره ولا ملفه في التخزين ولا تعليقه.
- **Auth UX وأمان 49/49**: توجيه `/account` الثلاثي، حماية الإدارة، حراسة مناطق الأعضاء مع وجهة العودة، ثلاث وجهات خارجية تُسقط، anon لا يقرأ 7 جداول خاصة ويقرأ 5 عامة ولا يكتب، عضو لا ينشئ ملف إدارة ولا يعلّق غيره ولا يقرأ جدول الإدارة، صفر أسرار في 20 حزمة متصفح، ولا اعتماد على مفاتيح قديمة.

### ثلاثة اكتشافات

1. **[HIGH] محسِّن الصور يتجاوز فحص الإخفاء.** `/community/media/…` يقطع فورًا عند الإخفاء (مؤكد 404)، لكن `/_next/image?url=/community/media/…` ظل يخدم الصورة المخفية **طوال 300 ثانية من القياس بلا توقف**، وترويسته `public, max-age=14400` — أي أربع ساعات في كاش مشترك، والرابط نفسه موجود في `srcset` الصفحة فهو متاح لأي أحد. هذا يُضعف ضمان Medium #9 من باب آخر. **لم أُصلحه**: العلاج يعني إخراج وسائط المجتمع من المحسِّن (`unoptimized`) في نحو ستة مواضع، وهو مقايضة أداء لمنصة تصوير — قرار مالك لا قرار مُنفِّذ.
2. **[LOW] `x-content-type-options: nosniff` لا يصل العميل على الإنتاج.** التطبيق يرسلها (مؤكد محليًا على نفس الإيداع)، وCDN هوستنجر يُسقطها بينما يُبقي `content-type` و`cache-control`. لا إصلاح برمجي ممكن — إعداد بنية تحتية.
3. **[LOW، أُصلح] الرئيسية كانت الصفحة الوحيدة بلا canonical** — لم تكن تعلن `metadata` إطلاقًا. أُضيف `alternates` واختبار انحدار يحرس أن كل صفحة عامة تُعلن metadata تُعلن معها `alternates`.

### التنظيف

كل جداول المجتمع صفر · `auth.users` = 2 (الموظفان) · صفر مجلدات في `community-media` · `bm-media` 14 كائنًا و`media` 14 صفًا بلا تغيير · اسم الموقع والعنوان عادا حرفيًا · `social_links` أربعة صفوف معطّلة كما كانت · صفر رسائل وطلبات.

## Task 45 — إغلاق ثغرة كاش محسِّن الصور (Open High)

اختار المالك الإغلاق لا قبول النافذة.

- **العلة المقيسة**: `/community/media/…` يقطع الوصول فورًا عند الإخفاء، لكن `/_next/image?url=/community/media/…` ظل يخدم صورة منشور مخفى **طوال 300 ثانية متواصلة بلا انقطاع**، بترويسة `public, max-age=14400` — أربع ساعات في كاش مشترك. ورابط المحسِّن مكتوب في `srcset` الصفحة، فلا يحتاج كشفُه شيئًا. المحسِّن يحتفظ بنسخته ويخدمها دون أن يسأل عقد الظهور مرة أخرى.
- **البحث كان شاملًا لا على المواضع الستة المعروفة**: 45 استخدامًا لـ`<Image>` في المشروع، فُحصت كلها. سبعة منها فقط يمكن أن تستقبل وسائط مجتمع (منشور، بورتفوليو ×2، محرر الملف ×2، المحجوبون، مؤلِّف المنشور). بقية المواضع صور ثابتة أو `bm-media`. والملف العام وMemberAvatar كانا يستخدمان `<img>` عادية أصلًا فلا يمران بالمحسِّن.
- **الحل**: مكوّن واحد `CommunityImage` يتخذ القرار من الرابط لا من المستدعي — `unoptimized` حين يبدأ المصدر بـ`/community/media/` أو `blob:`. بديل ذِكر `unoptimized` في سبعة مواضع ونسيانه في الثامن.
- **ما لم يُمَس**: `next.config` بلا تغيير (`bm-media` يبقى محسَّنًا عبر `remotePatterns`)، ومسار الوسائط بلا تغيير، وRLS والتخزين بلا تغيير، ولا روابط موقّعة.
- **حارس الانحدار** `tests/community-image-optimizer.test.ts` — 13 اختبارًا على أربع طبقات: صحة القرار نفسه · لا ملف مجتمع يستورد `next/image` مباشرة · مسح شامل لكل `<Image>` في المشروع بحثًا عن حقول روابط المجتمع أو بادئتها · وأي ملف يستورد من طبقة المجتمع لا يُبقي `<Image>` خامة. وأُثبت أنه يفشل بإعادة الثغرة عمدًا (استيراد `next/image` في post-card): فشل اختباران، وعاد 13/13 بالاسترجاع.

### تحقق حي بعد الإصلاح

- **المتصفح يثبت البديل**: في الخلاصة والملف العام، كل صور المجتمع تُحمَّل من `/community/media/…` مباشرة، **بلا `srcset` وبلا أي `/_next/image`** — العداد صفر على الصفحتين. وصور الموقع (الشعار) ما زالت تمر بالمحسِّن. مؤكد على سطح المكتب والموبايل، صفر صور مكسورة وصفر تجاوز أفقي.
- **الإشراف فوري على صورة حقيقية يعالجها CDN هوستنجر** (يعيد ترميزها 102KB ← 2KB): الإخفاء يقطع خلال أقل من ثانية ونصف، وستّ محاولات متتالية بعده لم تُسرّب نسخة واحدة. إلغاء نشر المشروع يقطع، والحذف يقطع، وأربعة مسارات غير مصرّح بها تُرفض 404.
- **سلوك العودة**: الرفض (404) يُخزَّن لحظيًا في CDN، فالطلب المتكرر أثناء الإخفاء يُبقيه 404 — وهذا الاتجاه الآمن. وبعد الاسترجاع تعود الصورة فور انتهاء ذلك التخزين (قِيس: 200 بعد فترة هدوء، مع كاسر تخزين وبدونه). أما الصورة نفسها فلا تُخزَّن في كاش مشترك أصلًا: `private, max-age=60` بلا `age`.
- الانحدار: «عام أولًا» والمجتمع **53/53** · RLS الحي **38/38** · 245 اختبارًا · TypeScript 0 · ESLint 0 · بناء 70/70.

### عطل اكتشفته وأتلف بيانات إنتاج — وأُصلح الأثر لا السبب

محاولةُ تحرير دورة من اللوحة بحمولة غير صالحة (أثناء اختبار القبول السابق) **محت منهج «ورشة أساسيات التصوير» ومواعيدها**: أربعة أيام وثمانية عناصر وموعدان.

السبب في `writeCourseChildren`: يحذف **كل** مواعيد الدورة وأيام منهجها أولًا، ثم يُدرج البديل. وليس معاملة واحدة — فأي خطأ في مرحلة الإدراج يترك الدورة بلا منهج ولا مواعيد، بلا تراجع. ليس خاصًا بحمولتي: أي فشل تحقق أو قيد مرجعي بعد الحذف يُنتج الأثر نفسه لمالكٍ يحرر دورة من الواجهة.

**استُعيدت البيانات** من تعريف البذور — وهو مطابق حرفيًا لبقية الدورات في الإنتاج (عدد المواعيد وتواريخها متطابق في تسع دورات)، فالاستعادة ليست تخمينًا. مؤكد حيًا: زر الحجز والمواعيد القادمة والمنهج كلها عادت.

**السبب لم يُصلح**: جعل الحفظ معاملة ذرية يحتاج RPC واحدة في القاعدة — تغيير معماري خارج نطاق هذه المهمة. مسجَّل كعطل مفتوح.

## Task 46 — استعادة كلمة المرور

- **ثلاثة مسارات جديدة تحت `/community`** فيغطيها نطاق الـmiddleware القائم بلا توسيع: `/community/reset-password` (الطلب)، و`/community/reset-password/callback` (استقبال رابط البريد)، و`/community/reset-password/update` (تعيين الكلمة). ورابط «نسيت كلمة المرور؟» في صفحتي الدخول كلتيهما، كلٌّ يحمل وجهة عودته.
- **الاستقبال Route Handler لا صفحة**، وهذا ليس تفصيلًا: كتابة الكوكيز مكتومة بالتصميم داخل Server Component (`setAll` في `server.ts` يبتلع الخطأ)، فتبادل الرمز في صفحة ينجح ثم تضيع الجلسة. الكتابة متاحة في Route Handler وحده.
- **يقبل ثلاثة أشكال** لأن شكل الرابط يتبع إعداد المشروع وقالب البريد اللذين لا أملك تغييرهما: `?code=` (PKCE، وهو الافتراضي في ‎@supabase/ssr)، و`?token_hash=` (قالب TokenHash)، ورموز خلف `#` لا تبلغ الخادم أصلًا فيقرأها المتصفح كما يفعل مسار قبول الدعوة الإداري القائم.
- **لا تعداد حسابات**: رسالة واحدة تغطي «أُرسل» و«لا حساب» و«رفض المزود». خطأ المزود يُسجَّل للخادم بسببه فقط — بلا بريد ولا رمز — ويُكتم عن المتصفح. شكل البريد وحده يُردّ عليه صراحةً لأنه لا يكشف وجود حساب.
- **لا عميل خدمة في التدفق إطلاقًا.** `updateUser` تعمل على صاحب الجلسة وحده؛ عميل الخدمة كان سيعني القدرة على تغيير كلمة مرور أي أحد. والجلسة تُغلق بعد النجاح: جلسة وُلدت من رابط بريد لا يصح أن تصير جلسة مفتوحة.
- **الوجهة لا تُبنى على ترويسة الطلب في الإنتاج.** الترويسة يتحكم بها المرسِل، فبناء رابط بريد عليها يعني طلبًا مزوَّرًا يوجّه رابط استعادة إلى نطاق يملكه غيرنا. في الإنتاج نطاق الموقع حصرًا — ويضمن هذا ألا يخرج رابط إلى localhost. وفي التطوير وحده يُقبل أصل محلي جيد الشكل. ووجهة ما بعد النجاح قائمة سماح من صفحتي دخول لا حارس عام.
- حدّ معدل 3 طلبات/15 دقيقة لكل عنوان — أقسى من حد النماذج العامة لأن كل طلب يرسل بريدًا.
- الجزء بعد `#` يُمسح من شريط العنوان قبل أي شيء فلا يبقى رمز في السجل.

### التحقق

- **19/19 من طرف إلى طرف** على بناء إنتاجي وقاعدة حقيقية، **بلا إرسال بريد** (الرابط يُولَّد بـ`generateLink` اختبارًا): الاستقبال يثبّت الجلسة ويحوّل · النموذج يظهر لصاحبها · التأكيد غير المطابق والكلمة القصيرة مرفوضان · التحديث ينجح · **الدخول بالكلمة الجديدة يعمل والقديمة لا** · الجلسة أُغلقت · الرابط يُستهلك مرة واحدة · الرمز المزوَّر يُرفض · البريد غير المسجل يعيد الرسالة العامة نفسها.
- **29 اختبارًا ساكنًا** تحرس: لا خروج عن النطاق في الإنتاج مهما ادّعى الطلب · لا تمييز بين حالات الطلب · لا عميل خدمة ولا مفتاح سري في أي ملف من التدفق · لا تسجيل رمز أو كلمة مرور · الاستقبال Route Handler ودائم الديناميكية · وجهة التحويل من أصل الطلب لا من الرابط · ومصادقة المشروع القائمة لم تتغير.
- الوجهة الإنتاجية **مدرجة فعلًا** في Redirect URLs لدى Supabase — فحص قراءة أعاد الرابط كما طُلب بلا استبدال.
- الانحدار: «عام أولًا» والمجتمع 53/53 · RLS 38/38 · **274 اختبارًا** · TypeScript 0 · ESLint 0 · بناء 71/71.

### قيد معروف

PKCE يخزّن مُتحقِّق الرمز في كوكي المتصفح الذي طلب الاستعادة، فطلبٌ من الحاسوب وفتحٌ للرابط من الجوال يفشل ويُعرض «اطلب رابطًا جديدًا». مسار الاستقبال يقبل `token_hash` أيضًا، فلو بدّل المالك قالب بريد الاستعادة إلى `{{ .TokenHash }}` عمل الرابط عبر الأجهزة بلا تغيير كود.

## Task 47 — حفظ الدورة معاملة واحدة (إغلاق آخر HIGH)

- **العلة**: `writeCourseChildren` كان يحذف **كل** مواعيد الدورة وأيام منهجها ثم يُدرج البديل، عبر طلبات PostgREST منفصلة — كلٌّ معاملتها. فأي فشل بعد الحذف يترك الدورة بلا منهج ولا مواعيد بلا رجعة. وقع فعلًا على الإنتاج ومُحيت بيانات «ورشة أساسيات التصوير».
- **العلاج**: `public.save_course_atomic(uuid, jsonb, jsonb, jsonb)` — دالة plpgsql واحدة يجري جسمها في معاملة واحدة. الحفظ صار نداءً واحدًا بدل ثلاث كتابات.
- **ترقية لازمة اكتشفتها أثناء الفحص**: `homepage_upcoming_course.manual_session_id` مفتاح أجنبي بـ`ON DELETE SET NULL`، والرئيسية **تثبّت فعلًا** موعدًا من `portrait-photography`. فالحذف‑ثم‑الإدراج كان يفكّ هذا التثبيت صامتًا في كل حفظ لتلك الدورة. الدالة الآن تُطابق الأطفال **بالمعرّف**: القائم يُحدَّث في مكانه، والجديد يُدرَج، والغائب وحده يُحذف — فتبقى الهوية وتبقى الإشارات إليها.
- **المعرّفات المولَّدة في المحرر** (`day-…`، `session-…`) ليست UUID، فالدالة تميّزها وتدرجها جديدة. لا تغيير في واجهة الإدارة ولا في `CourseInput`.
- **الأمان**: `SECURITY INVOKER` عمدًا لا DEFINER — المستدعي عميل الخدمة من Server Action مرّ ببوابة `requirePermission`، وهو نموذج التفويض الحقيقي للمشروع، فلا حاجة لرفع صلاحية. `EXECUTE` لـ`service_role` وحده، و`REVOKE` صريح عن public وanon وauthenticated. وفي الجسم حارس عمق يتحقق من `private.has_permission('courses','edit')` لو مُنحت يومًا لدور مستخدم. و`search_path` مثبَّت على ''.
- **التزامن**: `SELECT … FOR UPDATE` على صف الدورة قبل أي كتابة — حفظان متزامنان لنفس الدورة يتسلسلان بدل أن يتداخل حذف أحدهما مع إدراج الآخر. قفل صفّي واحد، لا يعطّل الجدول.
- **بلا `EXCEPTION WHEN`** في الجسم عمدًا: التقاط الاستثناء يفتح معاملة فرعية ويبتلع الفشل، فينهار الضمان.

### الإثبات — 24/24 على دورة مؤقتة (لا على دورة حقيقية)

- **نجاح**: تعديل بيانات الدورة والمنهج والمواعيد معًا؛ المنهج صار 3 أيام و4 بنود، والموعد الباقي **حافظ على معرّفه**، والمحذوف من الحمولة أُزيل، ولا بنود يتيمة.
- **فشل متعمد**: موعد ينتهك `CHECK(registered_count <= capacity)` **بعد** كتابة المنهج كاملًا. النتيجة: الـRPC ترفض برمز 23514، و**بيانات الدورة والمنهج وبنوده والمواعيد لم تتغير حرفيًا** — بالمعرّفات والترتيب لا بالعدد وحده. لا إدراج جزئي، ولا حذف جزئي، ولا صفوف يتيمة في القاعدة كلها.
- **التفويض**: الزائر والعضو العادي لا يستطيعان استدعاءها، ولا أثر لمحاولتيهما.
- **التزامن**: حفظان متزامنان انتهيا بلا خطأ، والحالة النهائية **إحدى الحمولتين كاملةً لا مزيجًا**.
- حارس انحدار من 15 اختبارًا، أُثبت أنه يفشل بإعادة النمط (3 إخفاقات) ويعود 15/15 بالاسترجاع.

## Task 48 — إصلاح فشل حفظ «الدورة القادمة» (v1.0.1)

بلاغ المالك: حفظ الصفحة الرئيسية يفشل بـ
`invalid input syntax for type uuid: "session-mu2k59qy-s06wxe"`.

- **السبب الجذري في المخزن لا في القاعدة.** محرر الدورة يولّد معرّفات مؤقتة للمواعيد وأيام المنهج (`session-…`، `day-…`)، والقاعدة تعطيها UUID عند الحفظ. لكن `addCourse` و`updateCourse` و`updateCurriculum` و`updateSessions` كانت **تُرقّع المخزن بالمُدخَل المُرسَل** بدل سحبه من القاعدة، فيبقى المعرّف المؤقت في الذاكرة. قائمة «الموعد» في محرر الصفحة الرئيسية تقرأ من المخزن نفسه، فأرسلت المؤقت إلى عمود `uuid`.
- **مؤكَّد بالبيانات**: موعد دورة NANlite محفوظ صحيحًا بـUUID `235ab496-…`، و`homepage_upcoming_course` فيه `manual_course_id` صحيح و`manual_session_id = null` — أي أن الحفظ توقف عند الموعد وحده. القاعدة سليمة والمتصفح هو المتأخر.
- **ليست انحدارًا من الحفظ الذرّي**: الترقيع المحلي قديم، وقبل الذرّية كانت القاعدة تُعيد توليد المعرّفات في كل حفظ فكان المخزن متأخرًا دائمًا. الذرّية لم تُحدث العلة ولم تخفِها.
- **العلاج طبقتان**: الأربعة تسحب الآن من القاعدة بعد النجاح (`refreshFromDb`) كما يفعل `duplicateCourse` أصلًا — فتحمل معرّفات حقيقية. وحفظ الصفحة الرئيسية يرفض أي معرّف ليس UUID برسالة عربية تقول ما العمل («حدّث الصفحة ثم أعد الاختيار») بدل خطأ Postgres خام — يحمي لوحةً مفتوحة منذ ما قبل التحديث.
- حارس انحدار من 7 اختبارات، أُثبت فشله بإعادة العلة (4 إخفاقات) وعودته 7/7.

## Task 49 — تجربة الدورة الأونلاين (منشور)

إعادة تصميم تجربة الدورة الرقمية من البطاقة إلى الدرس: بطاقة تسجيل موحّدة،
ومسار واضح من «اشترك» إلى أول درس، ومعالجة حالات لا محتوى فيها بدل مشغّل فارغ.
التفاصيل في الالتزامات السابقة لهذا اليوم.

## Task 50 — المرحلة الثانية للدفع: الأساس والتسجيل المجاني وميسر (اختبار)

- **الأساس**: `course_payments` و`payment_webhook_events` و`course_payment_methods`،
  وحساب ضريبي بالهللات: السعر المعروض شامل الضريبة، والصافي يُستخرج بقسمة صحيحة
  (`floor(gross*10000/11500)`) بمقارنة باقٍ لا بتقريب عائم.
- **المجاني**: تسجيل ذاتي بلا صف دفع إطلاقًا.
- **ميسر**: فواتير مستضافة في وضع الاختبار وحده. الإشعار **لا يفعّل شيئًا** — يُسجَّل
  ثم يُسأل المزوّد مباشرة، والتفعيل من الجواب الموثوق لا من جسم الطلب.
- **فشل مغلق**: مفتاح ناقص أو سر إشعار أقصر من 16 محرفًا ⇒ لا زر دفع ولا معالجة إشعار.

## Task 51 — تصحيح قاعدة العمل: نوع التسليم لا يقرر

قاعدة «الأونلاين وحدها تُسجَّل ذاتيًا» أُلغيت. كل دورة عادية — حضورية أو أونلاين أو
ورشة — تقبل التسجيل والدفع المباشر. **تدريب الشركات وطلب عرض السعر وحدهما** خارج
ذلك، ومسارهما واتساب.

## Task 52 — حجز المقاعد: المقعد الأخير تحميه القاعدة

- **العلة**: الورشة الحضورية سعتها حقيقية، والتسجيل كان يزيد عدّادًا في التطبيق.
  يكفي ما دام أحد يضغط في كل مرة، ويفشل حين يضغط عشرة معًا على آخر مقعد.
- **العلاج**: `course_session_seats` — المقعد صفّ يُطالَب به داخل معاملة تقفل الدفعة
  (`select … for update`)، ويحرسه فهرسان جزئيان فريدان (مقعد حيّ واحد لكل شخص في
  الدفعة، وواحد في الدورة كلها).
- **الترتيب المالي**: الحجز **قبل** إنشاء صف الدفع. فشل إنشاء الفاتورة يُطلق المقعد
  فورًا، وموت الخادم تنتهي مهلته وحدها بعد 20 دقيقة. مهلة الدفع 15 دقيقة — أقصر من
  الحجز عمدًا.
- **مدفوع بلا مقعد**: دفعة موثّقة وصلت بعد ضياع المقعد تُسجَّل `paid` بصدق وتُوسَم
  `seat_unavailable`: لا تفعيل وصول، ولا بيع مقعد مرتين، والقرار للإدارة.
- **لقطة مالية**: `course_payments` يحفظ اسم الدفعة وتاريخها وموقعها وقت الشراء، فلا
  يضيع معنى الدفعة لو عُدّلت الدورة أو حُذف الموعد لاحقًا.
- **حرّاس**: حذف دفعة لها تسجيلات مرفوض (`session_has_seats`)، وخفض السعة تحت المشغول
  مرفوض (`capacity_below_taken`) — من اللوحة ومن أي كتابة مباشرة.
- **سعة صفر** = «لم تُضبط» لا «بلا حدّ». و«ملغاة» حالة قائمة بذاتها لا مرادف لـ«مغلقة».

### الإثبات — 27/27 حيًّا على قاعدة الإنتاج ببيانات مؤقتة

- **عشرة مستخدمين متزامنين على سعة 1**: مقعد مؤكَّد واحد وتسجيل واحد، وتسعة يقرأون
  «اكتملت مقاعد هذا الموعد». وفي المسار المدفوع: حجز حيّ واحد و**صف دفع واحد** — لا
  يُطلب مال لمقعد لا نملكه.
- نقر مكرر ولسان تبويب ثانٍ: مقعد واحد، ولا خطأ قيد فريد يظهر للمستخدم.
- دفعة دورة أخرى · ملغاة · مغلقة · ماضية · مسودة · شركات · سعة غير مضبوطة: كلها مرفوضة
  برمز خاص بكل حالة.
- دفعة متأخرة بعد أخذ المقعد: `paid` + `seat_unavailable` + لا تسجيل + لا بيع مزدوج.
- مُنهيان متزامنان وإشعار مكرر: نتيجة واحدة لا تتغير.
- حجز منتهٍ يتوقف عن استهلاك مقعد **بلا مهمة دورية**.
- الحذف وخفض السعة مرفوضان، و`save_course_atomic` لا يُسقط دفعة محجوزة صامتًا.
- بمفتاح النشر وحده: لا قراءة ولا كتابة ولا نداء — إلا دالة التوافر المجمّعة عمدًا.

### علّتان كشفهما الاختبار الحيّ ولم يكشفهما أي اختبار وحدة

- **خطأ القاعدة كان يصل المستخدم نصًّا عامًا.** خطأ supabase-js كائن `{message, code}`
  لا `Error`، و`String(error)` يعطي `[object Object]` — فضاع كل سبب دقيق. وأسوأ:
  قراءة التوافر قبل المطالبة كانت ترد «لا توجد مواعيد» على من اختار موعدًا امتلأ للتو.
- **«12 مقاعد»** خطأ نحوي يقرؤه كل زائر. `formatSeats` صارت المصدر الوحيد للصيغة.

### حدّ معروف

تشغيل الاختبار الحيّ ينشئ دورات **منشورة** لثوانٍ. لو صادف ذلك بناء الاستضافة أو نافذة
الكاش (`s-maxage=300`) ظهرت في «دورات ذات صلة» وعلى الرئيسية أطول من عمرها في القاعدة.
حدث فعلًا في هذه المهمة وعولج بإعادة بناء. الأصح مستقبلًا: قاعدة تجريبية منفصلة.

## Task 53 — وسم سعر واحد لكل دورة

- **العلة**: دورة مجانية منشورة تعرض «حسب الطلب» بخط عريض ثم «مجانية» تحتها. الوسم
  كان يُشتق من الرقم — `formatPrice(0)` يعيد «حسب الطلب» — بينما «مجانية» تأتي من
  `enrollment.mode`. مصدران لسؤال واحد، فظهر جوابان متناقضان على البطاقة نفسها.
- **العلاج**: الحالة التجارية صارت حقلًا في نوع الدورة العام (`commercial`)، يُشتق
  مرة واحدة في الجسر من الحقول المعتمدة بالسلّم نفسه الذي يقرر به الخادم ما يُباع:
  الشركات أولًا، ثم `is_free`، ثم السعر. و`coursePriceDisplay` تعيد **مخرَجًا واحدًا**
  (وسم + سطر توضيحي)، فلا تستطيع واجهة أن تجمع وسمين.
- **القاعدة المعتمدة**: مجانية ← «مجانية». مدفوعة ← السعر شاملًا الضريبة وسطر الضريبة
  تحته. شركات أو طلب عرض سعر ← «حسب الطلب» وطريق واتساب بلا زر دفع. وسعر صفر بلا علَم
  مجانية ← «حسب الطلب» أيضًا: بيانات ناقصة لا هديّة.
- **المواضع**: البطاقة (القائمة والرئيسية والدورات ذات الصلة وصفحة المسار)، وصفحة
  الدورة، و«الدورة القادمة»، ومعاينة الرئيسية في اللوحة — والمعاينة تستعمل وسم الزائر
  نفسه لأنها تَعِد بأنها تُري ما سيُنشر.
- **خارج النطاق عمدًا**: سعر باقة المسار (`paths-view`) مجموع أسعار بخصم، لا حالة
  تجارية لدورة. تحقّقت أن المسارين المنشورين يعرضان مبلغين حقيقيين بلا تناقض.

### التحقق

- **17 اختبارًا جديدًا** يحرس الحالات الأربع: المجانية لا تقول «حسب الطلب» أبدًا،
  والمدفوعة لا تقول «مجانية»، والشركات «حسب الطلب»، ولا وسمين متناقضين في أي حالة —
  ومنها أن السلّم في الواجهة يطابق `commercialMode` على الخادم صفًا بصف.
- **على بناء إنتاجي وبيانات المالك الحقيقية** (بلا إنشاء أي بيانات مؤقتة): الدورة
  المجانية «مجانية» وحدها · المدفوعة «1,000 ريال» + سطر الضريبة بلا أي وسم آخر ·
  الشركات «حسب الطلب» بلا زر دفع مع واتساب · 12 بطاقة في القائمة كلٌّ بوسم واحد ·
  الرئيسية و«الدورة القادمة» والدورات ذات الصلة · جوال 375 بلا تجاوز أفقي · وحدة
  التحكم نظيفة.
- انحدار: TypeScript 0 · ESLint 0 · **558 اختبارًا** · بناء إنتاجي.

### قيد بيئي

`@next/swc-win32-x64-msvc` صار محجوبًا بسياسة Application Control على جهاز التطوير،
فتعذّر بناء Turbopack محليًا. البناء تمّ بـ`next build --webpack` ونجح. لا أثر على
الاستضافة (لينكس بارتباطاتها الأصلية)، والعائق في الجهاز لا في الكود.
