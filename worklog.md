...
- **اللوحة ← الموقع الحي**: التعديل يمر بـServer Action الحقيقي بجلسة إدارة حقيقية (بوابة `requirePermission`)، ثم يُقرأ الموقع العام. اسم الموقع والعنوان: حُفظا، ظهرا حيًا، ثم استُرجعا حرفيًا. **ملاحظة تشغيلية: `/courses` و`/about` تعكسان التعديل خلال ثانية، بينما الرئيسية قد تتأخر حتى 5 دقائق** — CDN هوستنجر يخدمها بـ`s-maxage=300`. قِيس: ظهرت خلال ثانية واحدة عند أول طلب بعد انتهاء الكاش.
