...
> قيد المالك الحالي (2026-09-12): هذه الوثيقة سجل تاريخي، وليست إذنًا لتنفيذ runbook أدناه. Community مطبق على Production؛ migration repair وإعادة migrations وأوامر تعديل السجل محظورة دون طلب جديد صريح. لم يُتحقق مجددًا من وضع السجل البعيد في جلسة التوثيق.
...
> **تصحيح 2026-09-12:** هذا القسم كان خاطئًا. اختُبر عمليًا فسقط: المدخلات
> remote-only **تمنع `db push` تمامًا** بخطأ `LegacyDbPushMissingLocalError`.
> ما دون ذلك من وصف التكرار صحيح. النص الأصلي مُبقى أدناه للسجل.
...
## ما جرى فعليًا (2026-09-12)

بإذن المالك نُفّذ الـrunbook، وانكشف أثناءه ما لم توثقه هذه الصفحة:

1. **الملفات غير المسجلة خمسة لا ثلاثة.** أُضيف `20260909090000_service_role_private_grants`
   و`20260909091000_contact_messages`. تحقق مستقل: جدول `contact_messages` موجود على
   الإنتاج، فالملف مطبَّق وغير مسجل. الاقتصار على الثلاثة كان سيُبقي `db push` فاشلًا
   عند `create policy` في `contact_messages`.
2. **`migration repair --status applied` نُفّذ للخمسة.** النتيجة: تسع ترحيلات
   `local = remote`، وثلاثة محلية غير مطبقة هي الترحيلات الجديدة وحدها.
3. **`db push --dry-run` فشل رغم ذلك** بسبب ثلاثة عشر مدخلًا remote-only — وهو ما
   يدحض القسم المصحَّح أعلاه.
4. **البديل المعتمد:** تنفيذ ملفات الترحيل الجديدة بـ`supabase db query --linked -f`
   ثم تسجيلها بـ`migration repair --status applied`. لا shadow migrations ولا
   `INSERT` يدوي ولا `--status reverted` — الممنوعات باقية كما هي.
5. نُفِّذت الترحيلات الثلاثة الجديدة بهذه الطريقة وسُجِّلت. الحالة الآن:
   **اثنا عشر ترحيلًا متزامنًا وصفر معلّق**.

### الأثر على المستقبل

`db push` يبقى مسدودًا ما دامت الثلاثة عشر قائمة. كل ترحيل قادم يسلك المسار رقم 4.
فتح `db push` يتطلب قرار مالك مستقلًا برفع الحظر عن `migration repair --status reverted`،
ولم يُطلب ولم يُنفَّذ.
